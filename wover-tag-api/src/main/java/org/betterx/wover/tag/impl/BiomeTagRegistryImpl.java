...
public class BiomeTagRegistryImpl extends TagRegistryImpl<Biome, TagBootstrapContext<Biome>> implements BiomeTagRegistry {
    ...
    public TagKey<Biome> makeStructureTag(ResourceKey<Structure> baseStructure) {
        return makeTag(Identifier.fromNamespaceAndPath(
                baseStructure.identifier().getNamespace(),
                "has_structure/" + (baseStructure.identifier().getPath())
        ));
    }
...
//    public void apply(Map<Identifier, List<TagLoader.EntryWithSource>> tagsMap) {
//        super.apply(tagsMap);
//    }
}
