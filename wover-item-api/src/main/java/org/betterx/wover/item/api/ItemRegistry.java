...
public abstract class ItemRegistry {
    ...
    public SpawnEggDefinition<SpawnEggItem> defineSpawnEgg(
        ...
        return defineSpawnEgg(
                eggName,
                (def) -> new SpawnEggItem(def.getProperties())
        );
    }
    ...
    public SmithingTemplateItem registerSmithingTemplateItem(
        ...
        // SmithingTemplates.Builder.build() bakes the item's own registry key via Item.Properties#setId,
        // and the client's item-model resolution follows that baked-in id rather than whatever key this
        // call uses - the two must match exactly or the item renders as missing/placeholder. Both sides go
        // through itemPath(), which appends vanilla's _smithing_template suffix.
        return registerSmithingTemplateItem(SmithingTemplates.Builder.itemPath(path), item);
    }
    ...
    public void bootstrapItemTags(ItemTagBootstrapContext ctx) {
        ...
        items
                ...
                .forEach(i -> ((ItemTagProvider) i.getValue()).registerItemTags(i.getKey().identifier(), ctx));
    }
}
